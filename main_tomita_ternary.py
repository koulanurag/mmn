...
class ObsNet(nn.Module):
    ...
    def encode(self, input):
        return self.bin_encoder(input) if self.bin_encoder is not None else input

    def decode(self, input):
        return self.bin_decoder(input) if self.bin_decoder is not None else input
...
def get_args():
    ...
    parser.add_argument('--ox_size', type=int, default=0, help="binary encoding size")
    ...
    parser.add_argument('--result_dir', default=os.path.join(os.getcwd(), 'results', 'Tomita', 'Ternary'),
...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.bgru_train or args.bgru_test or args.generate_fsm:
            gru_net = GRUNet(len(obs), args.gru_size, int(env.action_space.n))
            bhx_net = BNNet(args.gru_size, args.bhx_size)
            ox_net = ObsNet(gru_net.input_c_features, args.ox_size)
