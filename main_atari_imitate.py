...
from functions import BinaryTanh, TernaryTanh, DynamicGNoise
...
class _ObsBNNet(nn.Module):
    """Bottle Neck Network """

    def __init__(self, input_size, x_features):
        super(_ObsBNNet, self).__init__()
...
class ObsBNNet(nn.Module):
    """ Observation Bottle Neck Network"""

    def __init__(self, channels, input_size, x_features):
        """

        :param channels: No. of channels, there would be sub-network for each channel
        :type input_size: Flat size of features for the channels
        :param x_features: Tuple having the same length as the no. of channels.
                           This indicates no. of x-features to be used for the specific channel
        ...
        for i, x in enumerate(x_features):
            layer = _ObsBNNet(input_size, x)
            setattr(self, 'channel_net_{}'.format(i), layer)

    def forward(self, input):
        output, x = None, None
        for c in range(self.channels):
            decoded, encoded = getattr(self, 'channel_net_{}'.format(c))(input[:, c, :])
            encoded = encoded.unsqueeze(1)
            decoded = decoded.unsqueeze(1)
            output = decoded if c == 0 else torch.cat([output, decoded], dim=1)
            x = encoded if c == 0 else torch.cat([x, encoded], dim=1)
        return output, x

    def encode(self, input):
        x = None
        for c in range(self.channels):
            _, encoded = getattr(self, 'channel_net_{}'.format(c))(input[:, c, :])
            encoded = encoded.unsqueeze(1)
            x = encoded if c == 0 else torch.cat([x, encoded], dim=1)
        return x

    def decode(self, input):
        """ reconstruction from x-features"""
        output = None
        for c in range(self.channels):
            decoded, _ = getattr(self, 'channel_net_{}'.format(c))(input[:, c, :])
            decoded = decoded.unsqueeze(1)
            output = decoded if c == 0 else torch.cat([output, decoded], dim=1)
        return output
...
class HxBNNet(nn.Module):
    ...
    def __init__(self, input_size, x_features):
        ...
        self.bhx_size = x_features
        self.bin_encoder = nn.Sequential(nn.Linear(input_size, 5 * x_features),
                                         # nn.BatchNorm1d(5*x_features),
                                         nn.ELU(),
                                         nn.Linear(5 * x_features, 3 * x_features),
                                         # nn.BatchNorm1d(3 * x_features),
                                         nn.ELU(),
        ...
        self.bin_decoder = nn.Sequential(nn.Linear(x_features, 3 * x_features),
                                         # nn.BatchNorm1d(3 * x_features),
                                         nn.ELU(),
                                         nn.Linear(3 * x_features, 5 * x_features),
                                         # nn.BatchNorm1d(5 * x_features),
                                         nn.ELU(),
                                         nn.Linear(5 * x_features, input_size),
                                         nn.Tanh())
        self.apply(weights_init)
...
class GRUNet(nn.Module):
    def __init__(self, input_size, gru_cells, total_actions):
        ...
        self.conv1 = nn.Conv2d(input_size, 16, 3, stride=2, padding=1)
        self.conv2 = nn.Conv2d(16, 8, 3, stride=2, padding=1)
        self.conv3 = nn.Conv2d(8, 4, 3, stride=2, padding=1)
        self.conv4 = nn.Conv2d(4, 4, 3, stride=2, padding=1)

        # self.input_ff = nn.Sequential(self.conv1, F.elu, self.conv2, F.elu, self.conv3, F.elu, self.conv4, F.elu)
        self.input_ff = nn.Sequential(self.conv1, nn.ELU(), self.conv2, nn.ELU(), self.conv3, nn.ELU(), self.conv4,
                                      nn.ELU())
        self.input_c_shape = (4, 5, 5)
        self.input_c_features = 4 * 5 * 5
    ...
    def forward(self, input, input_fn=None, hx_fn=None, inspect=False):
        ...
        if input_fn is not None:
            b, c, f1, f2 = c_input.shape
            input, input_x = input_fn(c_input.reshape(b, c, f1 * f2))
            input = input.view(-1, self.input_c_features)
        else:
            c_input = c_input.view(-1, self.input_c_features)
            input, input_x = c_input, c_input

        # print('Cont Input')
        # print(c_input.view(-1, self.input_c_features).data.numpy().tolist())
        # print('BBN Input')
        # print(input.data.numpy().tolist())
        # print(nn.MSELoss()(c_input.view(-1,self.input_c_features),input).item())
...
class BinaryGRUNet(nn.Module):
    def __init__(self, input_size, gru_cells, total_actions):
        ...
        self.noise = DynamicGNoise(self.input_c_features, cuda=True, std=1)
        self.gru = nn.GRUCell(self.input_c_features, gru_cells)
...
#         _, _, (_, _, _, input_x) = self.gru_net((obs, hx), input_fn=self.obx_net, hx_fn=self.bhx_net, inspect=True)
#         return input_x
...
def get_args():
    ...
    parser.add_argument('--bn_episodes', type=int, default=10,
    ...
    parser.add_argument('--result_dir', default=os.path.join(os.getcwd(), 'results', 'Atari_Imitate'),
    ...
    try:
        ...
        if args.gru_train or args.gru_test:
            ...
            if args.gru_train:
                ...
                generate_trajectories(env, 32, args.batch_size, gru_prob_data_path, gru_net.cpu())
        ...
        if args.bgru_train or args.bgru_test or args.generate_fsm:
            gru_net = GRUNet(len(obs), args.gru_size, int(env.action_space.n))
            gru_net = gru_net.load_state_dict(torch.load(gru_net_path))
            bgru_net = BinaryGRUNet(len(obs), args.gru_size, int(env.action_space.n))
            bgru_net = bgru_net.load_state_dict(torch.load(gru_net_path))

            if args.cuda:
                bgru_net = bgru_net.cuda()
                # gru_net = gru_net.cuda()

            bgru_dir = ensure_directory_exits(os.path.join(env_dir, ''))
            ...
            if args.bgru_train:
                ...
                if args.gru_scratch:
                    ...
                    train_data = generate_trajectories(env, 50, args.batch_size, gru_prob_data_path,
                                                       copy.deepcopy(gru_net).cpu())
            ...
            if args.bgru_test:
                bgru_net.eval()
                bgru_perf = bgru_nn.test(bgru_net, env, 10, log=True, cuda=args.cuda, render=True)
            ...
            if args.generate_fsm:
                ...
                moore_machine.extract_from_nn(env, bgru_net, 50, 0, log=True)
