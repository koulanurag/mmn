...
class ObsBNNet(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.kmeans.predict(input)
        return input, torch.Tensor(x).unsqueeze(0)
    ...
    def decode(self, input):
        output = self.kmeans.cluster_centers_[int(input[0].item())]
...
def get_args():
    ...
    parser.add_argument('--batch_size', type=int, default=64, help="batch size used for training GRU")
    ...
    parser.add_argument('--result_dir', default=os.path.join(os.getcwd(), 'results','Atari_Clustering'),
    ...
    try:
        ...
        if args.gru_train or args.gru_test:
            ...
            if args.gru_train:
                ...
                # generate_trajectories(env, 200, args.batch_size, gru_prob_data_path, gru_net.cpu())
        ...
        if args.ox_test or args.ox_train:
            ...
            obs_train_data = np.array(obs_train_data).reshape(len(obs_train_data), 100).tolist()
            obs_test_data = np.array(obs_test_data).reshape(len(obs_test_data), 100).tolist()

            # kmeans = KMeans(n_clusters=10, random_state=0).fit(obs_train_data)
            #
            # ox_net = ObsBNNet(kmeans)
            # Compute Affinity Propagation
            af = AffinityPropagation().fit(obs_train_data)
            cluster_centers_indices = af.cluster_centers_indices_
            labels = af.labels_
            ox_net = ObsBNNet(af)
            pickle.dump(ox_net,open(ox_net_path,'wb'))
            # if args.ox_train:
        ...
        if args.bgru_train or args.bgru_test or args.generate_fsm:
            ...
            _channels = gru_net.input_c_shape[0]
            channel_features = int(np.prod(gru_net.input_c_shape[1:]))
            ox_net = pickle.load(open(ox_net_path,'rb'))
            ...
            if not args.bx_scratch:
                ...
                #
                #     bgru_net.obx_net = ObsBNNet(kmeans)
            ...
            if args.generate_fsm:
                ...
                moore_machine.extract_from_nn(env, bgru_net, 4, 0, log=True)
