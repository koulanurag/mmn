...
from functions import BinaryTanh
...
class BNNet(nn.Module):
    ...
    def __init__(self, input_size, x_features):
        ...
        self.bin_encoder = nn.Sequential(nn.Linear(input_size, 10),
                                         nn.ELU(),
                                         nn.Linear(10, x_features),
                                         BinaryTanh())

        self.bin_decoder = nn.Sequential(nn.Linear(x_features, 10),
                                         nn.ELU(),
...
class BinaryGRUNet(nn.Module):
    ...
    def forward(self, input, inspect=False):
        input, hx = input
        actor, hx, (ghx, bhx, input_c, input_x) = self.gru_net((input, hx), input_fn=self.obx_net, hx_fn=self.bhx_net,
                                                               inspect=True)
        if inspect:
            return actor, hx, (ghx, bhx), (input_c, input_x)
        else:
            return actor, hx
...
def generate_bottleneck_data(net, env, episodes, save_path, cuda=False):
    if os.path.exists(save_path):
        ...
        hx_data, obs_data = pickle.loads(open(save_path, "rb").read())
    else:
        ...
        pickle.dump((hx_data, obs_data), open(save_path, "wb"))

    h_threshold = int(0.7 * len(hx_data))
    o_threshold = int(0.7 * len(obs_data))

    return hx_data[:h_threshold], hx_data[h_threshold:], obs_data[:o_threshold], obs_data[o_threshold:]
...
def get_args():
    ...
    parser.add_argument('--gru_size', type=int, default=5, help="No. of GRU Cells")
    ...
    parser.add_argument('--bhx_size', type=int, default=10, help="binary encoding size")
    ...
    parser.add_argument('--bn_episodes', type=int, default=1000,
    ...
    try:
        ...
        if args.gru_train or args.gru_test:
            ...
            if args.gru_train:
                ...
                generate_trajectories(env, 500, args.batch_size, gru_prob_data_path, gru_net.cpu())
                write_net_readme(gru_net, gru_dir, info={'time_taken': time.time() - start_time})
            if args.gru_test:
                logging.info('Testing GRU!')
                gru_net.load_state_dict(torch.load(gru_net_path))
                gru_net.eval()
                perf = gru_nn.test(gru_net, env, 1000, log=True, cuda=args.cuda)
        ...
        if args.ox_test or args.ox_train:
            ...
            gru_net = GRUNet(len(obs), args.gru_size, int(env.action_space.n))
            ox_net = BNNet(gru_net.input_c_features, args.ox_size)
            if args.cuda:
                gru_net.cuda()
                ox_net = ox_net.cuda()
        ...
        if args.bgru_train or args.bgru_test or args.generate_fsm:
            ...
            if args.bgru_test:
                bgru_net.eval()
                bgru_perf = bgru_nn.test(bgru_net, env, 1000, log=True, cuda=args.cuda)
