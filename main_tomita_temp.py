...
class GRUNet(nn.Module):
    def __init__(self, input_size, gru_cells, total_actions):
        ...
        self.gru_cell = nn.GRUCell(self.input_c_features, gru_cells)
        self.gru_wrap = nn.Sequential(
                                 nn.Linear(gru_cells, gru_cells),
                                 BinaryTanh())
        # self.gru = nn.GRU(self.input_c_features, gru_cells, num_layers=2, bias=False)
        ...
        self.gru_cell.bias_ih.data.fill_(0)
        self.gru_cell.bias_hh.data.fill_(0)
    ...
    def forward(self, input, input_fn=None, hx_fn=None, inspect=False):
        ...
        ghx = self.gru_wrap(self.gru_cell(input, hx))
    ...
    if os.path.exists(save_path):
        ...
        with torch.no_grad():
            for seed in range(batches):
                data_obs, data_actions, data_len = [], [], []
                ...
                _train_data[seed] = (data_obs, data_actions, data_len)
        logging.info('Average Performance: {}'.format(sum(all_ep_rewards) / len(all_ep_rewards)))
...
def get_args():
    ...
    parser.add_argument('--bn_episodes', type=int, default=10000,
    ...
    parser.add_argument('--result_dir', default=os.path.join(os.getcwd(), 'results', 'TomitaTemp'),
    ...
    try:
        ...
        if args.gru_train or args.gru_test:
            ...
            if args.gru_train:
                ...
                # generate_trajectories(env, 1000, args.batch_size, gru_prob_data_path, gru_net.cpu())
