...
def _train(net, env, optimizer, batch_data, batch_size, cuda=False, grad_clip=1):
    cross_entropy_loss = nn.CrossEntropyLoss().cuda() if cuda else nn.CrossEntropyLoss()
    data_obs, data_actions = batch_data
    ...
    for i in range(env._max_episode_steps):
        ...
        loss += cross_entropy_loss(output, data_actions[:, i])
